...
- HTML is parsed with html5lib (reference HTML5 parser; no regex) in fragment mode. A **lexical-preservation layer** runs before parsing: character references and bare `&` are replaced by sentinel markers and every tag's exact source spelling is recorded, so for VALID input the output restores raw tag spelling (`<br>` vs `<br/>`, case, attribute order/quotes), entity spelling (`&nbsp;` ≠ `&#160;` ≠ `&#xA0;` ≠ `&amp;`), and source positions byte-for-byte — see the inline-code guarantees under Structured configuration. **Malformed input** is deterministically normalized (stray end tags dropped, implied end tags applied, tags canonicalized) — byte identity is NOT claimed for malformed input. A leading `<!DOCTYPE ...>` is preserved verbatim.
