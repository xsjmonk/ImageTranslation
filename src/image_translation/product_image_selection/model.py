...
def load_checkpoint(path, model, expected=None):
    ...
    required=("artifact_schema_version","state_dict","architecture","pretrained_weights","labels","input_size","normalization","temperature","taken_threshold","config_sha256","model_version","torch_version","torchvision_version","cuda_version","quality_gate","created_utc")
    ...
    if not isinstance(item["temperature"], (int,float)) or item["temperature"] <= 0:
        raise ArtifactError("invalid checkpoint temperature")
    if not isinstance(item["taken_threshold"], (int,float)) or not 0 <= item["taken_threshold"] <= 1:
    ...
    if not all(k in item["normalization"] for k in ("mean","std","padding_rgb")):
        raise ArtifactError("checkpoint preprocessing is incomplete")
    if len(item["normalization"]["mean"]) != 3 or len(item["normalization"]["std"]) != 3 or len(item["normalization"]["padding_rgb"]) != 3:
